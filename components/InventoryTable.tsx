  ...
  return (
    <>
      <div className="p-6 bg-slate-900/30 border border-slate-800/80 rounded-2xl shadow-2xl relative overflow-hidden text-slate-200 backdrop-blur-md">
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-800/60 bg-slate-950/20 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-800/80 transition-colors pr-1">
          {filteredProducts.length === 0 ? (
            ...
            <table className="w-full border-collapse text-left table-auto min-w-[1000px]">
              <thead>
                <tr className="border-b border-slate-800 text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-900/10 select-none"><th className="px-6 py-3.5 font-semibold text-left">Product Name</th><th className="px-4 py-3.5 font-semibold text-left w-32">Classification</th><th className="px-4 py-3.5 font-semibold text-left w-32">SKU</th><th className="px-4 py-3.5 font-semibold text-right w-28">Cost Price</th><th className="px-4 py-3.5 font-semibold text-right w-28">Retail Sale</th><th className="px-4 py-3.5 font-semibold text-right w-28">Stock Qty</th><th className="px-6 py-3.5 font-semibold text-center w-28">Status</th><th className="px-6 py-3.5 font-semibold text-right w-44">Actions</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-800/40 text-xs font-medium">
                {filteredProducts.map((product) => (
                  <tr key={product.id} className="group hover:bg-slate-900/30 transition-colors duration-150"><td className="px-6 py-4 font-semibold text-slate-100 whitespace-normal max-w-xs">{product.productName}</td><td className="px-4 py-4 whitespace-nowrap"><span className={`px-2 py-0.5 rounded-md text-[9px] uppercase font-black tracking-wider border select-none ${product.assetCategory === "fixed_asset" ? "bg-blue-500/5 text-blue-400 border-blue-500/20" : "bg-purple-500/5 text-purple-400 border-purple-500/20"}`}>{product.assetCategory === "fixed_asset" ? "Asset" : "Consumable"}</span></td><td className="px-4 py-4 font-mono text-slate-400 tracking-tight whitespace-nowrap">{product.sku || "—"}</td><td className="px-4 py-4 font-mono text-right text-slate-400 whitespace-nowrap">{formatCurrency(product.purchasePrice)}</td><td className="px-4 py-4 font-mono text-right font-bold text-slate-300 whitespace-nowrap">{formatCurrency(product.retailPrice)}</td><td className="px-4 py-4 font-mono font-black text-right text-slate-100 whitespace-nowrap">{product.currentVolumeMlGrams?.toLocaleString("en-IN")}<span className="text-[10px] text-slate-500 lowercase font-sans font-normal ml-0.5">{product.unitType || "ml"}</span></td><td className="px-6 py-4 text-center whitespace-nowrap"><span className={`px-2 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-wider select-none ${getAlertBadgeColor(product.alertLevel)}`}>{product.assetCategory === "fixed_asset" ? "Good" : getAlertLabel(product.alertLevel)}</span></td><td className="px-6 py-4 text-right whitespace-nowrap"><div className="flex items-center justify-end gap-2"><button type="button" onClick={() => { setSelectedProduct(product); setShowEditModal(true); }} disabled={isPending} className="px-2 py-1.5 rounded-lg text-[11px] font-bold bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700/50 transition-all cursor-pointer flex items-center gap-1 disabled:opacity-30 disabled:pointer-events-none shadow-sm"><Edit2 size={11} />Edit</button><button type="button" onClick={() => { setSelectedProduct(product); setShowAdjustModal(true); }} disabled={isPending} className="px-2 py-1.5 rounded-lg text-[11px] font-bold bg-pink-950/30 hover:bg-pink-900/40 text-pink-400 border border-pink-900/40 transition-all cursor-pointer flex items-center gap-1 disabled:opacity-30 disabled:pointer-events-none shadow-sm"><SlidersHorizontal size={11} />Adjust</button><button type="button" onClick={() => handleDelete(product.id)} disabled={isPending} className="p-1.5 rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-950/20 border border-transparent hover:border-red-900/30 transition-all cursor-pointer disabled:opacity-30 disabled:pointer-events-none" title="Delete Catalog Row Entry"><Trash2 size={13} /></button></div></td></tr>
                ))}
              </tbody>
            </table>
          )}
