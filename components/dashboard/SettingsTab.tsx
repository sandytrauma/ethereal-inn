...
import { 
  ...
  ChevronRight, X, ShieldCheck, Loader2, Edit3, Lock
} from 'lucide-react';
  ...
  return (
    <div className="space-y-10 pb-32 font-sans max-w-2xl mx-auto px-4">
      ...
      <section className="bg-slate-900/40 p-8 rounded-[3rem] border border-white/5 text-center shadow-2xl backdrop-blur-sm relative overflow-hidden">
        ...
        <div className="w-24 h-24 bg-gradient-to-tr from-amber-400 to-orange-500 rounded-[2rem] mx-auto flex items-center justify-center text-slate-950 text-4xl font-black mb-6 shadow-2xl shadow-amber-400/20">
          {user.name?.[0] || 'U'}
        </div>
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-end px-4">
          <div>
            ...
            <p className="text-[9px] text-slate-500 uppercase font-black tracking-[0.2em] mt-1">{isAdmin ? "Personnel & Access Keys" : "Verified Credentials"}</p>
          </div>
          {isAdmin && (
            <button onClick={() => setIsAddModalOpen(true)} className="p-4 bg-amber-400 text-slate-950 rounded-2xl hover:bg-amber-300 active:scale-95 transition-all shadow-xl shadow-amber-400/20">
          ...
          {visibleStaff.map((staff) => (
            <motion.div layout key={staff.id} className="flex items-center justify-between p-5 bg-slate-900/60 rounded-[2.5rem] border border-white/5 backdrop-blur-md">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-slate-800 rounded-2xl flex items-center justify-center text-sm font-black text-slate-400">{staff.name?.[0]}</div>
                <div>
                  <p className="text-sm font-black text-white">{staff.name} {Number(staff.id) === Number(user.id) && <span className="text-amber-400/50 ml-1 text-[10px]">(You)</span>}</p>
                  <p className="text-[9px] text-amber-400/60 uppercase font-black tracking-widest mt-0.5">{staff.role}</p>
                </div>
              </div>
              {isAdmin ? (
                <div className="flex gap-1">
                  <button onClick={() => handleRoleUpdate(staff.id, staff.role)} className="p-3 text-slate-500 hover:text-amber-400 hover:bg-amber-400/10 rounded-xl transition-all"><UserCog size={18} /></button>
                  {Number(staff.id) !== Number(user.id) && (
                    <button onClick={() => handleRemoveStaff(staff.id, staff.name)} className="p-3 text-slate-500 hover:text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all"><Trash2 size={18} /></button>
                  )}
                </div>
  ...
  return (
    <ModalWrapper title="New Identity" subtitle="Registering Staff Member" onClose={onClose}>
      <form onSubmit={handleSubmit} className="p-8 space-y-5">
        <Input label="Display Name" placeholder="John Doe" value={form.name} onChange={(v: string) => setForm({...form, name: v})} />
        <Input label="Email Address" type="email" placeholder="name@ethereal.inn" value={form.email} onChange={(v: string) => setForm({...form, email: v})} />
        <Input label="Access Key" type="password" placeholder="Min. 6 characters" value={form.password} onChange={(v: string) => setForm({...form, password: v})} />
        <div className="space-y-2 pb-4">
          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Authority Level</label>
          <select className="w-full bg-slate-900/50 border border-white/5 rounded-[1.5rem] p-5 text-white outline-none appearance-none text-sm font-bold" value={form.role} onChange={e => setForm({...form, role: e.target.value})}>
...
function EditProfileModal({ user, onClose }: { user: User, onClose: () => void }) {
  ...
  return (
    <ModalWrapper title="Update Profile" subtitle="Modify your credentials" onClose={onClose}>
      <form onSubmit={handleSubmit} className="p-8 space-y-5">
        <Input label="Display Name" value={form.name} onChange={(v: string) => setForm({...form, name: v})} />
        <Input label="New Password" type="password" placeholder="Leave blank to keep current" value={form.password} onChange={(v: string) => setForm({...form, password: v})} />
...
interface ModalWrapperProps { children: React.ReactNode; title: string; subtitle: string; onClose: () => void; }
function ModalWrapper({ children, title, subtitle, onClose }: ModalWrapperProps) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[150] flex items-end sm:items-center justify-center p-4 bg-slate-950/90 backdrop-blur-xl">
      <motion.div initial={{ y: 100, scale: 0.95 }} animate={{ y: 0, scale: 1 }} exit={{ y: 100, scale: 0.95 }} className="w-full max-w-md bg-[#020617] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl">
        <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/5">
          <div><h3 className="text-white font-black text-xl tracking-tight">{title}</h3><p className="text-[9px] text-slate-500 uppercase font-black tracking-widest mt-1">{subtitle}</p></div>
...
interface InputProps { label: string; type?: string; placeholder?: string; value: string; onChange: (v: string) => void; }
function Input({ label, type = "text", placeholder, value, onChange }: InputProps) {
  return (
    <div className="space-y-2">
      ...
      <input type={type} required={type !== "password"} className="w-full bg-slate-900/50 border border-white/5 rounded-[1.5rem] p-5 text-white outline-none focus:border-amber-400/50 transition-all text-sm" placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} />
    </div>
  );
}

interface SubmitButtonProps { loading: boolean; text: string; }
function SubmitButton({ loading, text }: SubmitButtonProps) {
  return (
    <button disabled={loading} className="w-full bg-amber-400 text-slate-950 font-black py-6 rounded-[2rem] hover:bg-amber-300 transition-all flex items-center justify-center gap-3 disabled:opacity-50">
...
interface SettingsActionProps { icon: any; title: string; desc: string; onClick: () => void; danger?: boolean; loading?: boolean; }
function SettingsAction({ icon: Icon, title, desc, onClick, danger, loading }: SettingsActionProps) {
  return (
    <button onClick={onClick} disabled={loading} className={`w-full flex items-center justify-between p-6 rounded-[2.5rem] border transition-all active:scale-[0.98] group ${danger ? 'bg-rose-500/5 border-rose-500/10 hover:bg-rose-500/10' : 'bg-slate-900/40 border-white/5 hover:bg-slate-800/60'}`}>
      <div className="flex items-center gap-5 text-left">
        <div className={`p-4 rounded-2xl transition-all ${danger ? 'text-rose-500 bg-rose-500/10' : 'text-amber-400 bg-amber-400/10 group-hover:scale-110'}`}><Icon size={22} className={loading ? 'animate-spin' : ''} /></div>
        <div><p className={`text-sm font-black tracking-tight ${danger ? 'text-rose-500' : 'text-white'}`}>{title}</p><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">{desc}</p></div>
      </div>
